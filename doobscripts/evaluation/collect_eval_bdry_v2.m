function [ bestF, bestP, bestR, bestT, F_max, P_max, R_max, Area_PR] = ...,
    collect_eval_bdry_v2(pbDir, append, overwrite)
% function [ bestF, bestP, bestR, bestT, F_max, P_max, R_max, Area_PR ] = ...,
% collect_eval_bdry_v2(pbDir, append, overwrite)
%
% modified from collect_eval_bdry(pbDir)
%
...
fname = fullfile(pbDir, ['eval_bdry',append,'.txt']);
if (length(dir(fname))==1) && ~overwrite
    ...
    Area_PR = 0;
    if(numel(R)>1)
        Area_PR = interp1(R, P, 0:.01:1);
        Area_PR = sum(Area_PR(~isnan(Area_PR)))/100;
    end    
    
    fname = fullfile(pbDir,['eval_bdry_img',append,'.txt']);
    fid = fopen(fname,'w');
    if fid==-1,
    ...
    fname = fullfile(pbDir,['eval_bdry_thr',append,'.txt']);
    fid = fopen(fname,'w');
    if fid==-1,
        error('Could not open file %s for writing.',fname);
    end
