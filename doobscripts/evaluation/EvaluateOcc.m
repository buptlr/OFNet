...
switch DataSet
    case 'PIOD'
        oriGtPath = '/data/DOOB/data/PIOD/Data/';
        oriResPath = '/home/xuefeng/project/DOOBNet-master/examples/doobnet/Output/';

        opt.method_folder = 'DOOBNet';
        opt.model_name = 'PIOD';
        
        testIdsFilename = '/data/DOOB/data/PIOD/val_doc_2010.txt';
        ImageList = textread(testIdsFilename, '%s'); omit_id = [];
        
    case 'BSDSownership'
        oriGtPath = '/data/DOOB/data/BSDSownership/testfg/';
        oriResPath = '/home/xuefeng/project/DOOBNet-master/examples/doobnet/Output/';
        
        opt.method_folder = 'DOOBNet';
        opt.model_name = 'BSDSownership';  
        
        testIdsFilename = '/data/DOOB/data/BSDSownership/Augmentation/test_ori_iids.lst';
        ImageList = textread(testIdsFilename, '%s'); omit_id = []; 
end


respath = [oriResPath, opt.method_folder, '/', opt.model_name , '/PIOD1g_5w'];
evalPath = [oriResPath, opt.method_folder, '/', opt.model_name , '/eval_fig1_5w/']; if ~exist(evalPath, 'dir') mkdir(evalPath); end
